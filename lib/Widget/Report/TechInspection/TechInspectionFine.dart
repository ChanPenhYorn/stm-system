...
class TechInspectionFine extends StatefulWidget {
  const TechInspectionFine({Key? key}) : super(key: key);

  @override
  State<TechInspectionFine> createState() => _TechInspectionFineState();
}

class _TechInspectionFineState extends State<TechInspectionFine> {
  @override
  void initState() {
  ...
  Future initData() async {
    ...
    source.buildYearData(jsonData[0]);
  }
                ...
                builder: (context, snapshot) {
                  if (snapshot.connectionState == ConnectionState.done) {
                    return Column(
                      children: [
                        Expanded(
                          child: ExtensionComponent.techInspectionFine(
                            zoom: false,
                            title: "សេវាពិន័យ",
                            techInspectionReportModel: jsonData[0],
                          ),
                        ),
                        //List
                        Container(
